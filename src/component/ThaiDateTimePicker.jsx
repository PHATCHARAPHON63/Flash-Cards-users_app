...
const ThaiDateTimePicker = ({
  ...
  const selectDate = (day) => {
    ...
    const selectedMoment = day.clone();
    // 👇 เพิ่มเวลาปัจจุบันลงไป
    selectedMoment.hour(moment().hour()).minute(moment().minute()).second(moment().second());
    if (onDateSelect) {
      onDateSelect(selectedMoment.toDate()); // ส่ง Date object ที่มีเวลา
    }
  };
